...
const DeleteProduksiPopUp = () => {
  const { setState, state } = globalState();
  const [isClose, setClose] = useState(false);
  const mutation = useDeleteProduksi()
  const xHandler = () => {
    setClose(!isClose);
    setState((prevState) => {
      return {
  ...
  const deleteProduksi = ()=>{
    if(state.getId !== undefined){
        mutation.mutate(state.getId)
    }
  }
  return (
    <div>
      <div
        className={`flex justify-center shadow-2xl rounded-md items-center gap-3 bg-white border py-2 px-4 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30 ${
          state.actionHapusProduksi ? "" : "hidden"
...
export default DeleteProduksiPopUp;
